...
# data_file.write('href,stress_address,city,state_and_zip,county,img_href,beds,bath,area,year_built,\
...
for i, href in enumerate(all_href):
    ...
    price = price.replace(',', '')
    area = area.replace(',', '')
    stress_address = driver.find_elements(
        By.CSS_SELECTOR, ".street-address")[0].text[:-1]
    city_and_state = driver.find_elements(
        By.CSS_SELECTOR, ".dp-subtext")[0].text
    ...
    if basic_infor:
        ...
        basic_infor_label = [e.text for e in basic_infor.find_elements(
            By.CSS_SELECTOR, "span.table-label")]
        basic_infor_value = [e.text for e in basic_infor.find_elements(
            By.CSS_SELECTOR, "div.table-value")]
        year_built = basic_infor_value[basic_infor_label.index(
            'Year Built')] if 'Year Built' in basic_infor_label else None
        county = basic_infor_value[basic_infor_label.index(
            'County')] if 'County' in basic_infor_label else None
    ...
    if scores:
        ...
        trademark = [score.find_elements(
            By.CSS_SELECTOR, ".walkscore-trademark")[0].text[:-1] for score in scores]
        score_value = [score.find_elements(By.CSS_SELECTOR, "div.percentage > span.value")[
            0].text for score in scores]
        walk_score = score_value[trademark.index(
            'Walk Score')] if 'Walk Score' in trademark else None
        transit_score = score_value[trademark.index(
            'Transit Score')] if 'Transit Score' in trademark else None
        bike_score = score_value[trademark.index(
            'Bike Score')] if 'Bike Score' in trademark else None

    property_details_container = driver.find_elements(
        By.CSS_SELECTOR, "div.super-group-content > div.amenity-group") or []
    ...
    for pc in property_details_container:
        ...
        if header == 'Heating & Cooling':
            heating_cooling_text = [
                e.text for e in pc.find_elements(By.TAG_NAME, "li")]
    ...
    photo_elements = driver.find_elements(
        By.CSS_SELECTOR, 'div.InlinePhotoPreview')
    photo_elements = photo_elements[0] if photo_elements else []
    img_href = " ".join([img.get_attribute('src')
                        for img in photo_elements.find_elements(By.TAG_NAME, "img")])

    # lot_size, cooling, heating, has_pool, parking_size
    data_file.write(f'{href},{stress_address},{city_and_state},{county},{img_href},{beds},{bath},{area},{year_built},{lot_size},{walk_score},{transit_score},{bike_score},{lot_size},{cooling},{heating},{has_pool},{parking_size},{price}\n')
